...
/* The program create_bmp_for_edge_coupled_coplanar_waveguide is a pre-processor for atlc. It produces bitmaps of differential waveguides
with no separate ground plane layer */
#include "config.h"
...
int main(int argc, char **argv) /* Read parameters from command line here   */
{
   ...
   if(argc-my_optind ==8)
   {
      ...
      Er2=pcb.Er2;
      if(pcb.WW<0)
        pcb.WW=6*pcb.hh+2*pcb.gg+2*pcb.ww+pcb.ss;
      if(pcb.HH<0)
        pcb.HH=6*(pcb.hh+pcb.tt);
      /* Do some sanaity checks */
      check_parameters_of_create_bmp_for_edge_coupled_coplanar_waveguide(pcb);
      convert_create_bmp_for_edge_coupled_coplanar_waveguide_dimensions_to_integers(&pcb);
      W=pcb.W;
      H=pcb.H;
      ...
      write_bitmap(image_data_fp, pcb);
       ...
       usage_create_bmp_for_edge_coupled_coplanar_waveguide();
   return(0);
}
