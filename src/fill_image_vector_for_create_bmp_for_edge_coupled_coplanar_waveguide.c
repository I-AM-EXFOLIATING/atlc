...
void fill_image_vector_with_data(unsigned char *image_vector, int colour_Er1, int colour_Er2, struct transmission_line_properties pcb)
{
   ...
   for(j=0;j<pcb.H;++j)
   {
	 ...
	 /* Put the dielectric for the top PCB in */
	 if((i >= BORDER) && (i <pcb.W-BORDER ) && (j >= pcb.H/2-pcb.t/2-pcb.h) && (j < pcb.H/2-pcb.t/2))
	 {
	 ...
	 /* Put the dielectric for the bottom PCB in */
	 if((i >= BORDER) && (i <pcb.W-BORDER ) && (j >= pcb.H/2+pcb.t/2) && (j < pcb.H/2+pcb.t/2+pcb.h))
	 {
	   red=colour_Er2/(256*256); /* Fill in areas with Er2 (normally PCB substrate)*/
	   green=colour_Er2/256;
	   blue=colour_Er2%256;
	 ...
	 if((i >= BORDER) && (i <=right_edge_of_LH_GP ) && (j >= pcb.H/2-pcb.t/2) && (j < pcb.H/2+pcb.t/2))
	 {
	 ...
	 if( (i > right_edge_of_LH_GP + pcb.g ) && (i <= right_edge_of_LH_GP + pcb.g+pcb.w) && (j >= pcb.H/2-pcb.t/2) && (j < pcb.H/2+pcb.t/2))
	 {
	   red=0xff;  /* Left hand coupler made to be +1 V */
	   green=0;
	   blue=0;
	 }
	 /* Right hand coupler, set to -1 V */
	 if( (i > right_edge_of_LH_GP + pcb.g +pcb.w+pcb.s) && (i <= right_edge_of_LH_GP + pcb.g+pcb.w+pcb.s+pcb.w) && (j >= pcb.H/2-pcb.t/2) && (j < pcb.H/2+pcb.t/2))
	 {
	   red=0;  /* Right hand coupler made to be -1 V */
	   green=0;
	   blue=0xff;
         }
	 if( (i > right_edge_of_LH_GP + pcb.g+pcb.w+pcb.s+pcb.w+pcb.g) && (i <= pcb.W-BORDER) && (j >= pcb.H/2-pcb.t/2) && (j < pcb.H/2+pcb.t/2))
	 {
